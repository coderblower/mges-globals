...
const Agency_Contract_Letter_Candidate_Single_With_Form = () => {
  ...
  const fetchCandidates = async () => {
    setLoading(true);
    try {
      const res = await post(`api/candidate_assign/get_all_Selected_Candidate_from_admin/${id}`, {
        page: currentPage,
        search,
                  ...
                  <tr key={item.id} className="whitespace-nowrap">
                    ...
                    <th>
                      <div className="flex gap-4 items-center justify-between">
                        ...
                        <NavLink to={`/hiring_country_recruting_agency/contract_letter/contract_letter_view/${item.id}`}>
                          <img
                            src={
                              item?.candidate?.approval_status === "reject" ||
                              item?.candidate?.approval_status === "pending"
...
export default Agency_Contract_Letter_Candidate_Single_With_Form;
